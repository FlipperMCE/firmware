 ...
 *  STATIC PROTOTYPES
 **********************/
static void ui_menu_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void ui_menu_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void ui_menu_page_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void ui_menu_page_destructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void ui_menu_cont_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);
static void ui_menu_section_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj);

const lv_obj_class_t ui_menu_class = {
    .constructor_cb = ui_menu_constructor,
    .destructor_cb = ui_menu_destructor,
    .base_class = &lv_obj_class,
    .width_def = (LV_DPI_DEF * 3) / 2,
    .height_def = LV_DPI_DEF * 2,
    .instance_size = sizeof(ui_menu_t)
};
const lv_obj_class_t ui_menu_page_class = {
    .constructor_cb = ui_menu_page_constructor,
    .destructor_cb = ui_menu_page_destructor,
    .base_class = &lv_obj_class,
    .width_def = LV_PCT(100),
    .height_def = LV_SIZE_CONTENT,
    .instance_size = sizeof(ui_menu_page_t)
};

const lv_obj_class_t ui_menu_cont_class = {
    .constructor_cb = ui_menu_cont_constructor,
    .base_class = &lv_obj_class,
    .width_def = LV_PCT(100),
    .height_def = LV_SIZE_CONTENT
};

const lv_obj_class_t ui_menu_section_class = {
    .constructor_cb = ui_menu_section_constructor,
    .base_class = &lv_obj_class,
    .width_def = LV_PCT(100),
    .height_def = LV_SIZE_CONTENT
};

const lv_obj_class_t ui_menu_separator_class = {
    .base_class = &lv_obj_class,
    .width_def = LV_SIZE_CONTENT,
    .height_def = LV_SIZE_CONTENT
};

const lv_obj_class_t ui_menu_sidebar_cont_class = {
    .base_class = &lv_obj_class
};

const lv_obj_class_t ui_menu_main_cont_class = {
    .base_class = &lv_obj_class
};

const lv_obj_class_t ui_menu_main_header_cont_class = {
    .base_class = &lv_obj_class
};

const lv_obj_class_t ui_menu_sidebar_header_cont_class = {
    .base_class = &lv_obj_class
};

static void ui_menu_refr(lv_obj_t * obj);
static void ui_menu_refr_sidebar_header_mode(lv_obj_t * obj);
static void ui_menu_refr_main_header_mode(lv_obj_t * obj);
static void ui_menu_load_page_event_cb(lv_event_t * e);
static void ui_menu_obj_del_event_cb(lv_event_t * e);
static void ui_menu_back_event_cb(lv_event_t * e);
static void ui_menu_value_changed_event_cb(lv_event_t * e);
/**********************
 *  STATIC VARIABLES
 **********************/
 ...
 *   GLOBAL FUNCTIONS
 **********************/
bool ui_menu_item_back_btn_is_root(lv_obj_t * menu, lv_obj_t * obj);
...
lv_obj_t * ui_menu_page_create(lv_obj_t * parent, const char * title)
{
    ...
    ui_menu_page_t * page = (ui_menu_page_t *)obj;
    if(title) {
...
{
    LV_LOG_INFO("begin");
    lv_obj_t * obj = lv_obj_class_create_obj(&ui_menu_cont_class, parent);
    lv_obj_class_init_obj(obj);
    return obj;
}
...
{
    LV_LOG_INFO("begin");
    lv_obj_t * obj = lv_obj_class_create_obj(&ui_menu_section_class, parent);
    lv_obj_class_init_obj(obj);
    return obj;
}
...
{
    LV_LOG_INFO("begin");
    lv_obj_t * obj = lv_obj_class_create_obj(&ui_menu_separator_class, parent);
    lv_obj_class_init_obj(obj);
    return obj;
}

void ui_menu_refr(lv_obj_t * obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);

    ui_menu_t * menu = (ui_menu_t *)obj;
    lv_ll_t * history_ll = &(menu->history_ll);

    /* The current menu */
    ui_menu_history_t * act_hist = _lv_ll_get_head(history_ll);
...
void ui_menu_set_page(lv_obj_t * obj, lv_obj_t * page)
{
    ...
    if(page != NULL) {
        /* Add a new node */
        lv_ll_t * history_ll = &(menu->history_ll);
        ui_menu_history_t * new_node = _lv_ll_ins_head(history_ll);
    ...
    if(menu->sidebar_page != NULL) {
        ...
        if(menu->sidebar_generated) {
            if(menu->mode_root_back_btn == UI_MENU_ROOT_BACK_BTN_ENABLED) {
    ...
    ui_menu_refr_main_header_mode(obj);
}
...
void ui_menu_set_sidebar_page(lv_obj_t * obj, lv_obj_t * page)
{
    ...
    if(page != NULL) {
        ...
        if(!menu->sidebar_generated) {
            /* Create sidebar */
            lv_obj_t * sidebar_cont = lv_obj_class_create_obj(&ui_menu_sidebar_cont_class, obj);
            ...
            lv_obj_t * sidebar_header = lv_obj_class_create_obj(&ui_menu_sidebar_header_cont_class, sidebar_cont);
            ...
            lv_obj_add_event_cb(sidebar_header_back_btn, ui_menu_back_event_cb, LV_EVENT_CLICKED, menu);
        ...
        ui_menu_refr_sidebar_header_mode(obj);
    }
    else {
...
void ui_menu_set_mode_header(lv_obj_t * obj, ui_menu_mode_header_t mode_header)
{
    ...
    if(menu->mode_header != mode_header) {
        ...
        ui_menu_refr_main_header_mode(obj);
        if(menu->sidebar_generated) ui_menu_refr_sidebar_header_mode(obj);
    }
}
...
void ui_menu_set_load_page_event(lv_obj_t * menu, lv_obj_t * obj, lv_obj_t * page)
{
    ...
    /* Remove old event */
    if(lv_obj_remove_event_cb(obj, ui_menu_load_page_event_cb)) {
        lv_event_send(obj, LV_EVENT_DELETE, NULL);
        lv_obj_remove_event_cb(obj, ui_menu_obj_del_event_cb);
    }

    ui_menu_load_page_event_data_t * event_data = lv_mem_alloc(sizeof(ui_menu_load_page_event_data_t));
    event_data->menu = menu;
    event_data->page = page;

    lv_obj_add_event_cb(obj, ui_menu_load_page_event_cb, LV_EVENT_CLICKED, event_data);
    lv_obj_add_event_cb(obj, ui_menu_obj_del_event_cb, LV_EVENT_DELETE, event_data);
}
...
bool ui_menu_back_btn_is_root(lv_obj_t * menu, lv_obj_t * obj)
{
    ...
    if(obj == ((ui_menu_t *)menu)->sidebar_header_back_btn) {
        return true;
    }

    if(obj == ((ui_menu_t *)menu)->main_header_back_btn && ((ui_menu_t *)menu)->prev_depth <= 1) {
        return true;
    }
...
static void ui_menu_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    menu->mode_header = UI_MENU_HEADER_TOP_FIXED;
    menu->mode_root_back_btn = UI_MENU_ROOT_BACK_BTN_DISABLED;
    menu->cur_depth = 0;
    menu->prev_depth = 0;
    menu->sidebar_generated = false;

    _lv_ll_init(&(menu->history_ll), sizeof(ui_menu_history_t));
    ...
    lv_obj_t * main_cont = lv_obj_class_create_obj(&ui_menu_main_cont_class, obj);
    ...
    lv_obj_t * main_header = lv_obj_class_create_obj(&ui_menu_main_header_cont_class, main_cont);
    ...
    lv_obj_add_event_cb(main_header_back_btn, ui_menu_back_event_cb, LV_EVENT_CLICKED, menu);
    ...
    lv_obj_add_event_cb(obj, ui_menu_value_changed_event_cb, LV_EVENT_VALUE_CHANGED, menu);
...
static void ui_menu_page_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    ui_menu_t * menu = (ui_menu_t *)lv_obj_get_parent(obj);

    lv_obj_set_parent(obj, ((ui_menu_t *)menu)->storage);
...
static void ui_menu_refr_sidebar_header_mode(lv_obj_t * obj)
{
    ...
    switch(menu->mode_header) {
        case UI_MENU_HEADER_TOP_FIXED:
            ...
            break;
        case UI_MENU_HEADER_TOP_UNFIXED:
            lv_obj_move_to_index(menu->sidebar_header, 0);
            lv_obj_set_flex_grow(menu->sidebar_page, 0);
            break;
        case UI_MENU_HEADER_BOTTOM_FIXED:
...
static void ui_menu_load_page_event_cb(lv_event_t * e)
{
    ...
    ui_menu_load_page_event_data_t * event_data = lv_event_get_user_data(e);
    ui_menu_t * menu = (ui_menu_t *)(event_data->menu);
    ...
    if(menu->sidebar_page != NULL) {
        ...
        if(sidebar) {
            ...
            ui_menu_clear_history((lv_obj_t *)menu);

            menu->selected_tab = obj;
        }
    }

    ui_menu_set_page((lv_obj_t *)menu, page);
}
...
static void ui_menu_back_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        if(ui_menu_back_btn_is_root((lv_obj_t *)menu, obj)) return;
        ...
        /* The previous menu */
        ui_menu_history_t * prev_hist = _lv_ll_get_next(history_ll, act_hist);
        ...
        if(prev_hist != NULL) {
            ...
            *  Remove it from the history because `ui_menu_set_page` will add it again */
            _lv_ll_remove(history_ll, prev_hist);
            menu->cur_depth--;
            ui_menu_set_page(&(menu->obj), prev_hist->page);
...
static void ui_menu_value_changed_event_cb(lv_event_t * e)
{
    ...
    ui_menu_page_t * main_page = (ui_menu_page_t *)ui_menu_get_cur_main_page(obj);
    ...
    ui_menu_page_t * sidebar_page = (ui_menu_page_t *)ui_menu_get_cur_sidebar_page(obj);
