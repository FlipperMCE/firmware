...
static uint8_t sd_write_buffer[SD_BLOCK_SIZE];
...
static uint32_t sectors_remaining = 0;
...
// Helper function to manage read-ahead scheduling
static void update_read_ahead(sd_op_t* next, uint32_t next_block) {
    if (sectors_remaining <= 1) {
        clear_op(next);
        return;
    }

    if (next->block_num != next_block || next->result != 1) {
        schedule_read(next, next_block);
    }
}
    ...
    if (sd_read_ops[0].result == 1) {
        *buffer = sd_read_ops[0].buffer;
        ...
        if (sd_read_ops[1].result == 1) {
            ...
            if (sectors_remaining > 1 && sd_read_ops[1].request == 0) {
                schedule_read(&sd_read_ops[1], next_block);
            }
        }
...
void __time_critical_func(gc_mmceman_get_write_block)(uint8_t** buffer) {
    *buffer = sd_write_buffer;
}
...
void gc_mmceman_block_task(void) {
    ...
    if (sd_write_op.request == 1) {
        uint32_t block_num = sd_write_op.start_block + sd_write_op.blocks_written;
